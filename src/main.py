...
gpt_data_analyzer_role = ("you need to distinguish text which is job description and which is redirects if it is job "
                       "description you just needs to return only 1 and if it is redirect you should return only href of link")
...
gpt_data_analyzer = SpeakWithGPT(system_role=gpt_data_analyzer_role) #For analyze plane text and find redirects if it exists
...
applications = web_scraper.turn_applications_into_list(web_scraper.APPLICATIONS_HTML)
...
useful_applications = Selector.select_application(gpt_model=gpt_application_analyzer,applications=evaluated_applications)
